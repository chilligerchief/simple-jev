...
- Each vision configuration has an actual example question/gold answer and a lazy-loaded source PNG with a verified hash, source attribution and dataset metadata. Images are copied without additional preprocessing into `assets/evaluations/images/`. The three POPE examples share an image. CIFAR-10 uses the clearer airplane case `image-000010`, chosen visually rather than by model outcome; its original 32×32 PNG is enlarged with nearest-neighbor-style pixel rendering. Other configurations use their first source case. These illustrative cases are not model outputs or selected wins. Image source terms remain those of the original datasets, not the JevBench MIT license.
