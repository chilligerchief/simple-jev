...
- Each vision configuration has an actual first-case question/gold answer and a lazy-loaded source PNG with a verified hash, source attribution and dataset metadata. Images are copied without additional preprocessing into `assets/evaluations/images/`. The three POPE examples share an image. These illustrative cases are not model outputs or selected wins. Image source terms remain those of the original datasets, not the JevBench MIT license.
